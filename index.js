window.addEventListener("load", () => {
  ...
  function drawCard() {
    ...
    const text = `<!DOCTYPE html><html> <head> <title>${card}</title> <meta charset="utf-8"> <meta name="viewport" content="width=device-width, initial-scale=1"><style>body{width:100vw;height:100vh;background-color:${colors[card]};}</style><h1>${card}</h1></body></html>`;
    const blob = new Blob([text], {type: "text/html"});
    const blobUrl = URL.createObjectURL(blob);
    window.open(blobUrl, '_blank', `popup,location,status,scrollbars,resizable,width=100,height=100,top=${x},left=${y}`);
